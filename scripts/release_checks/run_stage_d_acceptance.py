...
''')
    ...
    check('D082', '39 production elements', lambda: int(subprocess.check_output(['node', '--input-type=module', '--eval', "import {PRODUCTION_LIBRARY_COUNT} from './company_ui/products/visualizer/assets/production_library.mjs';console.log(PRODUCTION_LIBRARY_COUNT)"], cwd=ROOT, text=True).strip()) == 39)
