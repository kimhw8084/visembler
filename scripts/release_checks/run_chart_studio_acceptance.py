...
def confirm_name(page, value='Acceptance recipe'):
    page.locator('#cs-name-dialog').wait_for(state='visible')
    page.locator('#cs-name-input').fill(value)
    ...
    try:
      with tempfile.TemporaryDirectory(prefix='visembler-chart-studio-') as td:
        with NativeHost(ROOT,Path(td)/'data') as host, sync_playwright() as pw:
          ...
          page.locator('[data-column="1"]').click();page.locator('[data-action="insert-col-left"]').click();page.locator('[data-column-rename="1"]').click();confirm_name(page)
          ...
          for label,width,height in (('desktop-1440',1440,900),('narrow-1024',1024,900),('mobile-390',390,844),('reflow-200-equivalent',720,900)):
              ...
              receipt['responsive_evidence'][label]=page.evaluate('''()=>({viewport:{width:innerWidth,height:innerHeight},scroll:{width:document.documentElement.scrollWidth,height:document.documentElement.scrollHeight},canvas:(()=>{const r=document.querySelector('#cs-canvas')?.getBoundingClientRect();return r?{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height}:null})(),overflow:document.documentElement.scrollWidth>innerWidth+1,overflowMenu:document.querySelector('.cs-overflow')?.getBoundingClientRect().toJSON()||null})''')
