...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="visembler-p1-native-") as td:
        with NativeHost(ROOT, Path(td) / "data") as host, sync_playwright() as playwright:
            ...
            page.wait_for_url("**/visualizer/reports*", timeout=15000)
            page.get_by_label("Search reports").fill("P1 Managed")
            assert page.get_by_text("Active reports", exact=False).is_visible()
            page.get_by_role("button", name="Export JSON", exact=True).first.click()
            page.get_by_role("button", name="Open editor", exact=True).click()
            page.wait_for_url("**/visualizer?report=*")
            ready(page)
