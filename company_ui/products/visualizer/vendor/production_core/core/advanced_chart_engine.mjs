...
function commonSvg(type,width,height,title,body,desc=''){
  ...
  return `<svg class="viz-advanced-chart cs-chart-svg" data-chart-type="${esc(type)}" viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(title||type)}"><title>${esc(title||type)}</title>${desc?`<desc>${esc(desc)}</desc>`:''}${body}</svg>`;
}
