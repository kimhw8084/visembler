...
with sync_playwright() as p:
 b=p.chromium.launch(headless=True,executable_path='/usr/bin/chromium');pg=b.new_page(viewport={'width':1440,'height':1000});pg.on('pageerror',lambda e:print('ERR',e));pg.on('console',lambda m:print(m.type,m.text));pg.set_content(html,wait_until='load');pg.wait_for_timeout(300);print('cards',pg.locator('.gallery-card').count());pg.get_by_role('button',name='Dynamic Grid').click();print('layout',pg.locator('#slideStage .sim-item').count());pg.get_by_role('button',name='100k Grid').click();pg.wait_for_timeout(1500);print('data',pg.locator('.vg-row').count(),pg.locator('#gridMeta').inner_text());b.close()
