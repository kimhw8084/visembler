...
export function projectDataEntry(entry = {}, sourceDataset = {}, mapping = {}) {
  ...
  if (entry.engine === 'TimelineEngine') {
    ...
    return { ...resolved, milestones: rows.map((row, index) => ({ label: String(row[fieldIndex(dataset, mapping.category || mapping.label)] ?? index + 1), date: row[fieldIndex(dataset, mapping.time)] ?? null })), source_row_count: dataset.rows.length };
  }
  if (entry.engine === 'EngineeringChartEngine') {
    const rows = sampledRows(dataset.rows, PERFORMANCE_LIMITS.engineeringRows);
    const valueIndex = fieldIndex(dataset, mapping.value), labelIndex = fieldIndex(dataset, mapping.time || mapping.category || mapping.x), subgroupIndex = fieldIndex(dataset, mapping.subgroup);
  ...
  const labels = pick(mapping.category ? 'category' : mapping.label ? 'label' : mapping.time ? 'time' : 'x');
  ...
  if (entry.engine === 'MetricEngine') return { ...resolved, value: lastValue ?? null, unit: fieldById(dataset, valueId)?.unit || entry.unit || '' };
  ...
  const data = indexes.map(index => [String(labels[index] ?? index + 1), values[index]]);
