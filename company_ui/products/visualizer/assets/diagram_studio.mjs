...
function closeResponsiveOverflow(details, returnFocus=true) {
  if (!details?.open) return;
  details.open=false;
  if (returnFocus) requestAnimationFrame(()=>focusVisible(details.querySelector('summary')));
}
...
function fitViewport(action='fit-page',manual=true) {
  ...
  const node=activeNode(),bounds=action==='fit-selection'&&node?{x:node.x-30,y:node.y-30,w:node.width+60,h:node.height+60}:diagramBounds(state.diagram),availableWidth=Math.max(120,wrap.clientWidth-48),availableHeight=Math.max(120,wrap.clientHeight-48),baseScale=Math.min(1,availableWidth/Math.max(1,bounds.w)),target=action==='fit-width'?availableWidth/bounds.w:Math.min(availableWidth/bounds.w,availableHeight/bounds.h);
  state.zoom=clamp(target/Math.max(baseScale,.01),.25,1.5);state.viewportTouched=manual||state.viewportTouched;state.initialFitDone=true;svg.style.transform=`scale(${state.zoom})`;svg.style.transformOrigin='top left';
  requestAnimationFrame(()=>{wrap.scrollLeft=Math.max(0,bounds.x*state.zoom-24);wrap.scrollTop=Math.max(0,bounds.y*state.zoom-24);notify(`Fit diagram · ${Math.round(state.zoom*100)}%`);});
}
...
function renderCanvas() {
  ...
  const width=Math.max(320,bounds.x+bounds.w+40),height=Math.max(240,bounds.y+bounds.h+40); svg.setAttribute('viewBox',`0 0 ${width} ${height}`);svg.setAttribute('width',String(width));svg.setAttribute('height',String(height));svg.style.width='';svg.style.height='';svg.dataset.grid=state.grid?'on':'off';
...
function loadImport(file) { const reader=new FileReader();reader.onload=()=>{try{const value=JSON.parse(String(reader.result||''));const raw=value.diagram||value.model?.items?.find(item=>item.id===boot.element_id)||value;applyMutation('Import diagram',()=>raw.diagram ? diagramFromEntry(raw) : normalizeDiagram(raw));state.viewportTouched=false;requestAnimationFrame(()=>requestAnimationFrame(()=>fitViewport('fit-page',false)));}catch(error){notify(error.message||'Diagram JSON could not be imported','error');}};reader.readAsText(file); }
...
function execute(action, ...args) {
  ...
  if(action==='toggle-palette')return togglePanel('palette');if(action==='toggle-inspector')return togglePanel('inspector');if(action==='close-panels')return closePanels();
...
function bind() {
  ...
  $('#ds-panel-backdrop')?.addEventListener('mousedown',event=>event.preventDefault());
  root.addEventListener('click',event=>{const overflowAction=event.target.closest('[data-overflow-action]')?.dataset.overflowAction;if(overflowAction){event.preventDefault();event.stopImmediatePropagation();closeResponsiveOverflow(event.target.closest('[data-responsive-overflow]'),overflowAction!=='preview');execute(overflowAction);return;}const action=event.target.closest('[data-action]')?.dataset.action;if(!action)return;closeResponsiveOverflow(event.target.closest('[data-responsive-overflow]'),action!=='preview');},true);
  ...
  root.addEventListener('keydown',event=>{const node=event.target.closest?.('[data-node-id]');if(!node)return;if(event.key==='Enter'||event.key===' '||event.key==='Spacebar'){event.preventDefault();selectNode(node.dataset.nodeId,event.shiftKey||event.metaKey);return;}if(event.key==='F2'){event.preventDefault();beginInlineNodeEdit(node.dataset.nodeId);}},true);
  ...
  $('#ds-canvas-wrap')?.addEventListener('wheel',event=>{if(event.ctrlKey||event.metaKey||event.deltaY){event.preventDefault();state.viewportTouched=true;state.zoom=clamp(state.zoom*(event.deltaY<0?1.1:.9),.25,2);const svg=$('#ds-canvas');svg.style.transform=`scale(${state.zoom})`;svg.style.transformOrigin='top left';notify(`Zoom ${Math.round(state.zoom*100)}%`);}}, {passive:false});
