...
function fitViewport(action='fit-page',manual=true) {
  ...
  const node=activeNode(),bounds=action==='fit-selection'&&node?{x:node.x-30,y:node.y-30,w:node.width+60,h:node.height+60}:diagramBounds(state.diagram),availableWidth=Math.max(120,wrap.clientWidth-48),availableHeight=Math.max(120,wrap.clientHeight-48),target=action==='fit-width'?availableWidth/bounds.w:Math.min(availableWidth/bounds.w,availableHeight/bounds.h);
  state.zoom=clamp(target,.25,1.5);state.viewportTouched=manual||state.viewportTouched;state.initialFitDone=true;svg.style.transform=`scale(${state.zoom})`;svg.style.transformOrigin='top left';
