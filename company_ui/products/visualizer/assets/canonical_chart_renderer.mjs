...
function layoutFor(model, data, width, height) {
  ...
  const xLabelSpace=rotation===0?Math.min(42,Math.max(18,maxXLabel*.34)):Math.min(110,Math.max(32,maxXLabel*Math.sin(rad)+28));
  ...
  const legendWidth=legendEntries.length&&verticalLegend?Math.min(220,Math.max(118,Math.max(...legendItemWidth,118))):0;
  const left=Math.max(52,Math.min(170,Math.ceil(maxYLabel)+yTitleSpace+12)+(legendPosition==='left'?legendWidth:0));
  const right=Math.max(28, (model.axes?.y?.position==='right'?Math.min(90,Math.ceil(maxYLabel)+yTitleSpace):28)+(legendPosition==='right'?legendWidth:0));
  const top=38+(legendPosition==='top'?legendHeight:0), bottom=24+xLabelSpace+titleSpace+(legendPosition==='bottom'?legendHeight:0);
  const plotWidth=Math.max(120,width-left-right),plotHeight=Math.max(100,height-top-bottom);
  return {width,height,left,right,top,bottom,plotWidth,plotHeight,legendPosition,legendHeight,legendWidth,xLabelSpace,rotation,verticalLegend};
}
...
function buildLegend(plan, model, layout, theme) {
  ...
  entries.forEach((series,index)=>{
    ...
    if(vertical || cursorX+itemWidth>boxX+usable){cursorX=boxX;cursorY+=lineHeight;}
...
function renderBar(plan, theme) {
  ...
  const yScale=plan.yAt;
  data.xKeys.forEach((key,index)=>visible.forEach((series,visibleIndex)=>{const originalIndex=plan.series.indexOf(series),stack=plan.stack?.stacks[originalIndex]?.[index],value=stack?.value ?? data.bySeries[originalIndex]?.[index]?.y;if(!finite(value))return;const display=stack?.display ?? value,base=stack?.base ?? 0;const xPosition=horizontal?plan.valueAt(display,'primary'):plan.xAt(index,'category');const zero=horizontal?plan.valueAt(base,'primary'):plan.yAt(base,'primary');let x,y,w,h;if(horizontal){y=layout.top+index*band+(band-slot)/2;x=Math.min(xPosition,zero);w=Math.max(1,Math.abs(xPosition-zero));h=slot;}else{x=layout.left+index*band+(mode==='grouped'?visibleIndex*slot+(band-visible.length*slot)/2:(band-slot)/2);y=Math.min(xPosition,zero);w=mode==='grouped'?slot:slot;h=Math.max(1,Math.abs(xPosition-zero));}
    const label=axisFormat(model.axes.y,display,{percentDomain:plan.percentDomain}),full=`${series.name} · ${String(key)} · ${label}`;marks.push(`<rect class="cs-mark cs-bar-mark" data-chart-point="${data.bySeries[originalIndex]?.[index]?.sourceIndex ?? index}" data-behavior-point="${data.bySeries[originalIndex]?.[index]?.sourceIndex ?? index}" data-series-key="${esc(series.key)}" data-tooltip="${esc(full)}" tabindex="0" role="button" aria-label="${esc(full)}" x="${x.toFixed(2)}" y="${y.toFixed(2)}" width="${w.toFixed(2)}" height="${h.toFixed(2)}" fill="${series.color}" rx="2"><title>${esc(full)}</title></rect>`);
    if(model.visual?.dataLabels)marks.push(labelMarkup(plan,{x:x+w/2,y:y+(h>18?Math.min(18,h/2):0)},label,theme.ink,index));
  }));
  return marks.join('');
}
