...
function renderLineLike(plan, theme) {
  ...
  plan.series.forEach((series,seriesIndex)=>{if(!series.visible)return;const sourcePoints=data.rows.filter(row=>row.series===series.key),points=sourcePoints.map(point=>{const categoryIndex=plan.xNumeric?point.xValue:data.xKeys.indexOf(String(point.xRaw??'')),x=plan.xAt(categoryIndex,plan.xNumeric?'numeric':'category'),y=finite(point.y)?plan.yAt(point.y,series.axis):null;return {...point,x,y,value:point.y};});let segment=[];const flush=()=>{if(!segment.length)return;const line=pathFor(segment,series.smoothing),detailPoints=segment.length<=12?segment:[...segment.slice(0,6),...segment.slice(-6)],omitted=segment.length-detailPoints.length,detail=detailPoints.map(point=>`${xLabel(model,point.xRaw,point.x,plan.temporal)} ${axisFormat(series.axis==='secondary'?model.axes.secondaryY:model.axes.y,point.value,{percentDomain:series.axis==='primary'&&plan.percentDomain})}`).join(' · '),accessibleTitle=`${series.name} · ${segment.length} ${segment.length===1?'point':'points'} · ${detail}${omitted?` · ${omitted} additional points omitted`:''}`;if(plan.type==='Area Chart'){const first=segment[0],last=segment.at(-1),baseY=plan.yAt(baseline,series.axis);parts.push(`<path class="cs-area cs-series-${seriesIndex}" data-series-key="${esc(series.key)}" d="M${first.x.toFixed(2)} ${baseY.toFixed(2)} ${segment.map(point=>`L${point.x.toFixed(2)} ${point.y.toFixed(2)}`).join(' ')} L${last.x.toFixed(2)} ${baseY.toFixed(2)} Z" fill="${series.color}" fill-opacity="${model.visual?.areaOpacity ?? .25}"><title>${esc(series.name)} area</title></path>`);}parts.push(`<path class="cs-line cs-series-${seriesIndex}" data-series-key="${esc(series.key)}" role="img" aria-label="${esc(accessibleTitle)}" d="${line}" fill="none" stroke="${series.color}" stroke-width="${series.lineWidth}" stroke-dasharray="${series.lineDash==='dashed'?'8 5':series.lineDash==='dotted'?'2 4':'none'}" stroke-linecap="round" stroke-linejoin="round"><title>${esc(series.name)}</title></path>`);segment=[];};
