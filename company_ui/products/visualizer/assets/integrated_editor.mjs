...
function renderDataFirstDialog() {
  ...
  $('#modalBody').innerHTML=`<div class="modal-form data-first-dialog">${paste}<div class="data-first-summary"><b>${intake.rows.length.toLocaleString()} rows · ${intake.fields.length} columns</b><span>${intake.header.present?'Header detected':'No header detected'} · ${intake.delimiter==='\t'?'TSV':intake.delimiter===','?'CSV':'Delimited text'} · ${intake.warnings.length} warning${intake.warnings.length===1?'':'s'}</span></div><ul class="data-first-fields">${fields}</ul><section class="data-first-recommendations" aria-label="Production visual recommendations">${cards}</section><section class="data-first-mappings"><div><b>Field mapping</b><button type="button" class="link-button" id="dataFirstReset">Use detected mapping</button></div>${mappingRows}</section>${error}${replacement}<div class="modal-actions"><button type="button" class="tb" data-close>Cancel</button><button type="button" class="tb accent" id="dataFirstCreate" ${validation.valid?'':'disabled'}>Create visual</button></div></div>`;
  bindDataFirstDialog();
}
async function parseDataFirstText(text) {
  ...
  try { const intake=await parsePasteAsync(text); if(ui.dataFirst!==state||token!==state.token)return; state.intake=intake; state.loading=false; state.mapping=null; state.mappingView=null; renderDataFirstDialog(); }
...
function openDataFirstDialog() { ui.dataFirst={token:0,intake:null,loading:false,mapping:null,mappingView:null,view:null,destinationEntryId:ui.selected.size===1?[...ui.selected][0]:null}; renderDataFirstDialog(); }
