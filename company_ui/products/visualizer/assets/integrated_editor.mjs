...
function replaceDataset(entry, label, nextDataset, nextMapping=entry.mapping||{}) {
  const existing=selectedDataset(entry); if(existing&&datasetConsumers(existing.id).length>1){const detached={...nextDataset,id:nextDataset.id===existing.id?datasetId():nextDataset.id,revision:1};return commitOps(label,[{op:'model.patch',patch:{datasets:[...model().datasets,detached],crossFilter:null}},{op:'item.patch',id:entry.id,patch:{dataset_id:detached.id,mapping:nextMapping}}]);}
...
function openDatasetRefresh(entry){const dataset=selectedDataset(entry);if(!dataset)return;ui.datasetRefresh={entryId:entry.id,sourceText:'',intake:null};const render=()=>{const state=ui.datasetRefresh,linked=datasetConsumers(dataset.id).length,plan=state.intake?planDatasetRefresh({dataset,intake:state.intake,items:model().items,selectedId:entry.id,viewForEntry:viewContractForEntry}):null;$('#modalTitle').textContent='Refresh data';const summary=plan?`${dataset.rows.length} rows → ${state.intake.rows.length} rows · ${plan.valid?plan.compatibility.kind:plan.reason}`:'Paste a recurring export to check compatibility.';$('#modalBody').innerHTML=`<div class="modal-form data-first-dialog"><b>${esc(dataset.name)} · revision ${dataset.revision||1}</b><span>Feeds ${linked} visual${linked===1?'':'s'}</span><textarea id="refreshDataText" placeholder="Paste refreshed Excel, CSV, or TSV data">${esc(state.sourceText)}</textarea><small>${esc(summary)}</small><div class="modal-actions"><button class="tb" data-close>Cancel</button><button class="tb" id="refreshReview" ${state.intake?'':'disabled'}>Review mapping</button><button class="tb accent" id="refreshLinked" ${plan?.valid?'':'disabled'}>Refresh linked dataset · ${linked} visuals</button>${linked>1?`<button class="tb" id="refreshOnly" ${plan?.valid?'':'disabled'}>Refresh this visual only</button>`:''}</div></div>`;$('#refreshDataText').oninput=async event=>{state.sourceText=event.target.value;state.intake=await parsePasteAsync(state.sourceText);render();};$('#refreshLinked').onclick=()=>{if(commitDatasetRefresh(entry,state.intake,false))closeModals();};$('#refreshOnly').onclick=()=>{if(commitDatasetRefresh(entry,state.intake,true))closeModals();};$('#refreshReview').onclick=()=>{closeModals();openDataFirstDialog();ui.dataFirst.sourceText=state.sourceText;parseDataFirstText(state.sourceText);};};render();openModal($('#genericModal'),$('#refreshDataText'));}
