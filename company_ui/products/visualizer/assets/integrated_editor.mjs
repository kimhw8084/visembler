...
function commitDatasetRefresh(entry,intake,selectedOnly=false){const dataset=selectedDataset(entry);if(!dataset)return false;const plan=planDatasetRefresh({dataset,intake,items:model().items,selectedId:entry.id,selectedOnly,viewForEntry:viewContractForEntry});if(!plan.valid){toast(plan.reason);return false;}const nextId=selectedOnly&&plan.consumers.length>1?datasetId():dataset.id;const next={...datasetFromIntake(intake,nextId,dataset.name),revision:nextId===dataset.id?(dataset.revision||0)+1:1,metadata:{...(dataset.metadata||{}),refresh:{count:((dataset.metadata?.refresh?.count)||0)+1,previous_revision:dataset.revision||0,previous_row_count:(dataset.rows||[]).length,new_row_count:(intake.rows||[]).length,schema_signature:plan.schema_signature}}};const ops=[];if(nextId===dataset.id)ops.push({op:'model.patch',patch:{datasets:model().datasets.map(value=>value.id===dataset.id?next:value),crossFilter:null}});else ops.push({op:'model.patch',patch:{datasets:[...model().datasets,next],crossFilter:null}});for(const rebound of plan.mappings){const target=item(rebound.id),patch={mapping:rebound.mapping,...(nextId!==dataset.id?{dataset_id:nextId}:{})};if(nextId!==dataset.id&&target?.transform_recipe?.source_dataset_id)patch.transform_recipe={...target.transform_recipe,source_dataset_id:nextId};ops.push({op:'item.patch',id:rebound.id,patch});}return commitOps(selectedOnly?'Refresh this visual data':'Refresh linked dataset',ops,{announce:`Refreshed ${next.name}`});}
function openDatasetRefresh(entry){const dataset=selectedDataset(entry);if(!dataset)return;ui.datasetRefresh={entryId:entry.id,sourceText:'',intake:null};const render=()=>{const state=ui.datasetRefresh,linked=datasetConsumers(dataset.id).length,plan=state.intake?planDatasetRefresh({dataset,intake:state.intake,items:model().items,selectedId:entry.id,viewForEntry:viewContractForEntry}):null,detail=plan?.compatibility?.kind==='changed'?[plan.compatibility.added.length?`added ${plan.compatibility.added.join(', ')}`:'',plan.compatibility.removed.length?`missing ${plan.compatibility.removed.join(', ')}`:''].filter(Boolean).join(' · '):'';$('#modalTitle').textContent='Refresh data';const summary=plan?`${dataset.rows.length} rows → ${state.intake.rows.length} rows · ${plan.valid?plan.compatibility.kind:plan.compatibility.kind==='changed'?`Schema changed · ${detail}`:plan.reason}`:'Paste a recurring export to check compatibility.';$('#modalBody').innerHTML=`<div class="modal-form data-first-dialog"><b>${esc(dataset.name)} · revision ${dataset.revision||1}</b><span>Feeds ${linked} visual${linked===1?'':'s'}</span><textarea id="refreshDataText" placeholder="Paste refreshed Excel, CSV, or TSV data">${esc(state.sourceText)}</textarea><small>${esc(summary)}</small><div class="modal-actions"><button class="tb" data-close>Cancel</button><button class="tb" id="refreshReview" ${state.intake?'':'disabled'}>Review mapping</button><button class="tb accent" id="refreshLinked" ${plan?.valid?'':'disabled'}>${linked===1?'Refresh data':`Refresh linked dataset · ${linked} visuals`}</button>${linked>1?`<button class="tb" id="refreshOnly" ${plan?.valid?'':'disabled'}>Refresh this visual only</button>`:''}</div></div>`;$('#refreshDataText').oninput=async event=>{state.sourceText=event.target.value;state.intake=await parsePasteAsync(state.sourceText);render();};$('#refreshLinked')?.addEventListener('click',()=>{if(commitDatasetRefresh(entry,state.intake,false))closeModals();});$('#refreshOnly')?.addEventListener('click',()=>{if(commitDatasetRefresh(entry,state.intake,true))closeModals();});$('#refreshReview')?.addEventListener('click',()=>{closeModals();openDataFirstDialog();ui.dataFirst.sourceText=state.sourceText;parseDataFirstText(state.sourceText);});};render();openModal($('#genericModal'),$('#refreshDataText'));}
...
function viewContractForEntry(entry) { if(entry?.view_type)return entry.view_type; if(entry?.engine==='TableEngine')return 'table';if(entry?.engine==='DiagramEngine')return 'diagram';if(entry?.engine==='EngineeringChartEngine')return 'engineering';if(entry?.engine==='WaferFabEngine')return 'wafer';if(entry?.engine==='CoreChartEngine'&&/(line|area)/i.test(entry.element||''))return 'line';return 'bar'; }
...
async function parseDataFirstText(text) {
  const state=ui.dataFirst; if(!state)return; const token=++state.token; state.loading=true; state.intake=null; renderDataFirstDialog();
...
function bindDataFirstDialog() {
  ...
  $('[data-close]',form)?.addEventListener('click',closeModals,{once:true});
  $('#dataFirstText',form)?.addEventListener('input',event=>{const text=event.target.value;state.sourceText=text;if(text.trim())parseDataFirstText(text);});
...
function setLibrary(open){ui.libraryOpen=!!open;if(ui.libraryOpen&&mobileShell())ui.inspectorOpen=false;activeRoot?.setAttribute('data-library',ui.libraryOpen?'open':'closed');activeRoot?.setAttribute('data-inspector',ui.inspectorOpen?'open':'closed');const button=$('#libraryToggle');if(button){button.setAttribute('aria-pressed',ui.libraryOpen?'true':'false');button.setAttribute('aria-label',ui.libraryOpen?'Hide library':'Show library');button.textContent='Library';}storage.set('viz-library-open',ui.libraryOpen?'1':'0');requestAnimationFrame(()=>{if(ui.autoFit||ui.preview)fitZoom();else renderGeometryOnly();});}
function setInspector(open){ui.inspectorOpen=!!open;if(ui.inspectorOpen&&mobileShell())ui.libraryOpen=false;activeRoot?.setAttribute('data-inspector',ui.inspectorOpen?'open':'closed');activeRoot?.setAttribute('data-library',ui.libraryOpen?'open':'closed');const button=$('#inspectorToggle');if(button){button.setAttribute('aria-pressed',ui.inspectorOpen?'true':'false');button.setAttribute('aria-label',ui.inspectorOpen?'Hide inspector':'Show inspector');button.textContent='Inspector';}storage.set('viz-inspector-open',ui.inspectorOpen?'1':'0');requestAnimationFrame(()=>{if(ui.autoFit||ui.preview)fitZoom();else renderGeometryOnly();});}
