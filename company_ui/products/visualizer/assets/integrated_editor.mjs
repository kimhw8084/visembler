...
function renderDataFirstDialog() {
  ...
  const paste=`<label class="data-first-paste"><span>Paste rows from Excel, CSV, or TSV</span><textarea id="dataFirstText" placeholder="Paste rows from Excel, CSV, or TSV" aria-describedby="dataFirstHelp"></textarea><small id="dataFirstHelp">Paste a rectangular spreadsheet range, CSV, or TSV. Data stays in this report.</small></label>`;
  if(!intake) {
  ...
  const matches=state.matches||[], saved=state.savedPresetId&&mappingPresets.find(preset=>preset.id===state.savedPresetId), savedUi=matches.length===1&&saved?`<div class="data-first-saved" aria-live="polite"><b>Using saved mapping · ${esc(saved.name)}</b><button type="button" class="link-button" id="dataFirstDetected">Use detected mapping</button><button type="button" class="link-button" data-forget-mapping="${esc(saved.id)}" aria-label="Forget mapping ${esc(saved.name)}">Forget</button></div>`:matches.length>1?`<div class="data-first-saved"><b>Saved mappings for this data</b><div class="data-first-saved-list">${matches.map(match=>`<button type="button" class="tb" data-use-mapping="${esc(match.preset.id)}">${esc(match.preset.name)} · ${esc(productionTargetForView(match.preset.view)?.element||match.preset.view)}</button>`).join('')}</div></div>`:'';
...
function bindDataFirstDialog() {
  ...
  $('[data-close]',form)?.addEventListener('click',closeModals,{once:true});
  $('#dataFirstText',form)?.addEventListener('input',event=>{const text=event.target.value;if(text.trim())parseDataFirstText(text);});
...
function openDataFirstDialog() { ui.dataFirst={token:0,intake:null,loading:false,mapping:null,mappingView:null,view:null,matches:[],savedPresetId:null,destinationEntryId:ui.selected.size===1?[...ui.selected][0]:null}; renderDataFirstDialog(); }
