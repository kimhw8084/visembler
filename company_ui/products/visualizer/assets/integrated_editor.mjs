...
function openDataFirstDialog() { ui.dataFirst={token:0,intake:null,loading:false,sourceText:'',mapping:null,mappingView:null,view:null,matches:[],savedPresetId:null,destinationEntryId:ui.selected.size===1?[...ui.selected][0]:null}; renderDataFirstDialog(); }
...
function syncPanelState(){activeRoot?.setAttribute('data-library',ui.libraryOpen?'open':'closed');activeRoot?.setAttribute('data-inspector',ui.inspectorOpen?'open':'closed');syncPanelButtons();storage.set('viz-library-open',ui.libraryOpen?'1':'0');storage.set('viz-inspector-open',ui.inspectorOpen?'1':'0');requestAnimationFrame(()=>{if(ui.autoFit||ui.preview)fitZoom();else renderGeometryOnly();});}
function setLibrary(open){ui.libraryOpen=!!open;if(ui.libraryOpen&&mobileShell())ui.inspectorOpen=false;syncPanelState();}
function setInspector(open){ui.inspectorOpen=!!open;if(ui.inspectorOpen&&mobileShell())ui.libraryOpen=false;syncPanelState();}
