...
function wafer(entry){
  ...
  if(entry.dataset_id&&Array.isArray(entry.observations)&&entry.observations.length) { const observed=new Map(entry.observations.map(row=>[`${row.x}:${row.y}`,row.value])); const values=entry.observations.map(row=>Number(row.value)).filter(Number.isFinite);if(!values.length)return shell(entry,'<div class="data-empty-state full"><b>No numeric die values</b><span>Map a numeric measurement or yield field to the wafer value role.</span></div>','Fab');const min=Math.min(...values),max=Math.max(...values),range=max-min,level=value=>Number.isFinite(Number(value))?Math.max(0,Math.min(4,range===0?2:Math.round((Number(value)-min)/range*4))):0;const cells=[];for(let y=-4;y<=4;y+=1)for(let x=-5;x<=5;x+=1)if(x*x+y*y<28){const value=observed.get(`${x}:${y}`);cells.push(`<span class="die heat-${level(value)}" style="--x:${x+5};--y:${y+4}" title="${esc(value??'Missing')}"></span>`);}const identity=[['Wafer',entry.wafer_id],['Lot',entry.lot],['Tool',entry.tool],['Chamber',entry.chamber],['Recipe',entry.recipe],['Step',entry.process],['Bin',entry.bin]].filter(([,value])=>value!==null&&value!==undefined&&String(value).trim()!=='');return shell(entry,`<div class="wafer-demo"><div class="wafer-grid">${cells.join('')}</div><span class="wafer-notch"></span></div><div class="metric-meta"><span>${num(min)} → ${num(max)}${range===0?' · constant population':''}</span>${identity.map(([label,value])=>`<span>${esc(label)} · ${esc(value)}</span>`).join('')}</div>`,'Fab'); }
