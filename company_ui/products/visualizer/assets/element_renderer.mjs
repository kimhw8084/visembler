...
function comparison(entry){
  ...
  if(n.includes('time compression')) return shell(entry,`<div class="time-compression-live"><div><i></i><i></i><i></i><i></i><b>${num(entry.before,'188m')}</b></div><span>→</span><div class="short"><i></i><b>${num(entry.after,'14m')}</b></div></div>`,'Comparison');
...
function table(entry){
  ...
  if(bound?.headers?.length){
    ...
    if(!meaningful)return shell(entry,`<div class="table-frame empty-data"><table><thead><tr>${bound.headers.slice(0,4).map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead></table><div class="data-empty-state"><b>No rows yet</b><span>Paste data or open Data in the inspector.</span></div></div>`,'Table');
    return shell(entry,`<div class="table-frame"><table><thead><tr>${bound.headers.slice(0,4).map((h,i)=>`<th data-direct="${entry.dataset_id?'dataset-header':'table-header'}:${i}" title="Double-click to rename">${esc(h)}</th>`).join('')}</tr></thead><tbody>${(bound.rows||[]).map((row,rowIndex)=>`<tr>${bound.headers.slice(0,4).map((_,i)=>`<td data-direct="${entry.dataset_id?'dataset-cell':'table-cell'}:${rowIndex}:${i}" title="Double-click to edit">${num(row[i],'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,'Table');
  }
  if(entry.dataset_id&&entry.customTable?.headers?.length) return shell(entry,`<div class="table-frame"><table><thead><tr>${entry.customTable.headers.slice(0,4).map((h,i)=>`<th data-direct="dataset-header:${i}" title="Double-click to rename">${esc(h)}</th>`).join('')}</tr></thead><tbody>${(entry.customTable.rows||[]).map((row,r)=>`<tr>${entry.customTable.headers.slice(0,4).map((_,i)=>`<td data-direct="dataset-cell:${r}:${i}" title="Double-click to edit">${num(row[i],'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,'Table');
  ...
  if(c?.headers?.length){
    ...
    return shell(entry,`<div class="table-frame"><table><thead><tr>${c.headers.slice(0,4).map((h,i)=>`<th data-direct="table-header:${i}" title="Double-click to rename">${esc(h)}</th>`).join('')}</tr></thead><tbody>${(c.rows||[]).map((r,rowIndex)=>`<tr>${c.headers.slice(0,4).map((_,i)=>`<td data-direct="table-cell:${rowIndex}:${i}" title="Double-click to edit">${num(r[i],'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,'Table');
  }
...
function image(entry){
  ...
  if(n.includes('image + caption')) return shell(entry,`<figure class="captioned-image-live"><div id="${imageId}" style="${src}" role="img" aria-label="${alt}" aria-describedby="${captionId}">${entry.src?'':'Add image'}</div><figcaption id="${captionId}">${esc(entry.caption||'Caption: describe the image')}</figcaption></figure>`,'Media');
