...
function metric(entry){
  ...
  if(n==='hero kpi') {
    ...
    const context=hasTarget?`<span>Target ${renderValue(target)}</span>`:entry.period?`<span>${esc(entry.period)}</span>`:'<span>Current performance</span>';
    ...
    return shell(entry,`<div class="hero-kpi"><div class="metric-value">${value}</div><div class="metric-meta"><span>Delta ${num(entry.delta)}</span>${context}</div></div>`,'Metric');
  }
  if(n==='metric + delta') return shell(entry,`<div class="delta-metric"><b>${value}</b><span>Delta ${num(entry.delta)}</span><small>${esc(entry.period||'Comparison period')}</small></div>`,'Metric');
  if(n.includes('target')) {
    const actual=entry.actual??entry.value,target=entry.target,valid=finiteNumber(actual)&&finiteNumber(target);
    ...
    return shell(entry,`<div class="target-metric"><div class="metric-value">${renderValue(actual)}</div><div class="target-track"><i style="width:${position(actual)}%"></i><em style="left:${position(target)}%"></em></div><div class="metric-meta"><span>Target ${renderValue(target)}</span><span>Variance ${renderValue(variance)}</span></div></div>`,'Metric');
  }
  if(n.includes('progress')) { const current=entry.current??entry.value,max=entry.max??100,pct=boundedPercent(current,max); return shell(entry,`<div class="metric-value">${renderValue(current)}</div><div class="progress-hero"><i style="width:${pct??0}%"></i></div><div class="metric-meta"><span>${pct===null?'No numeric progress':`${Math.round(pct)}% complete`}</span><span>Max ${renderValue(max)}</span></div>`,'Metric'); }
  ...
  if(n.includes('status')) return shell(entry,`<div class="status-hero"><i></i><div><b>${esc(entry.status||'Unspecified')}</b><span>${value}</span></div></div><div class="metric-meta"><span>${esc(entry.detail||'')}</span></div>`,'Metric');
  if(n.includes('threshold')) { const warning=entry.warning,critical=entry.critical,current=entry.value,logic=entry.threshold_logic==='lower-is-worse'?'lower-is-worse':'higher-is-worse',scaleValues=[current,warning,critical].filter(finiteNumber),max=scaleValues.length?Math.max(...scaleValues,1):null,raw=boundedPercent(current,max),pos=raw===null?null:logic==='lower-is-worse'?100-raw:raw; return shell(entry,`<div class="metric-value">${value}</div><div class="threshold-scale ${logic}" data-threshold-logic="${logic}"><span></span><span></span><span></span>${pos===null?'':`<i style="left:${pos}%"></i>`}</div><div class="metric-meta"><span>Warning ${renderValue(warning)}</span><span>Critical ${renderValue(critical)}</span><span>${logic}</span></div>`,'Metric'); }
  if(n.includes('sparkline')) {
    ...
    return shell(entry,`<div class="metric-value compact">${value}</div><svg class="metric-spark" viewBox="0 0 180 48" role="img" aria-label="${values.length} numeric trend points"><path d="${path}"/></svg><div class="metric-meta"><span>${num(entry.delta)}%</span><span>${esc(entry.period||'last 7 periods')}</span></div>`,'Metric');
  }
  if(n.includes('ring')) { const max=entry.max??100,current=entry.value,pct=boundedPercent(current,max),dash=pct===null?'':`stroke-dasharray="${pct} ${100-pct}"`; return shell(entry,`<div class="metric-ring-large"><svg viewBox="0 0 100 100"><circle cx="50" cy="50" r="38" class="chart-ring-bg"/><circle cx="50" cy="50" r="38" class="chart-ring" pathLength="100" ${dash} transform="rotate(-90 50 50)"/></svg><b>${value}</b></div><div class="metric-meta center"><span>${esc(entry.center_label||'Progress')}</span></div>`,'Metric'); }
  if(n.includes('ladder')) { const levels=Array.isArray(entry.levels)&&entry.levels.length?entry.levels:[['P90',52],['Median',42.8],['P10',31]]; return shell(entry,`<div class="metric-ladder">${levels.slice(0,5).map((x,i)=>{const label=Array.isArray(x)?x[0]:x.label,val=Array.isArray(x)?x[1]:x.value,formatEntry=!Array.isArray(x)&&x&&typeof x==='object'?{...entry,...x}:entry;return `<div class="l${i}"><span>${esc(label)}</span><b>${renderValue(val,formatEntry)}</b></div>`;}).join('')}</div>`,'Metric'); }
  if(n.includes('capacity')) { const current=entry.current??entry.value,capacity=entry.capacity??100,pct=boundedPercent(current,capacity); return shell(entry,`<div class="capacity-metric"><b>${renderValue(current)}</b><div><i style="width:${pct??0}%"></i><em></em></div><span>${pct===null?'No numeric capacity ratio':`${Math.round(pct)}% of ${renderValue(capacity)} capacity`}</span></div>`,'Metric'); }
  if(n.includes('rate')) { const hasBasis=entry.numerator!==null&&entry.numerator!==undefined&&entry.denominator!==null&&entry.denominator!==undefined,basis=hasBasis?`${num(entry.numerator)} / ${num(entry.denominator)}`:value; return shell(entry,`<div class="rate-metric"><b>${basis}</b>${entry.value===null||entry.value===undefined?'':`<span>Rate ${value}</span>`}<i>↗</i><small>${esc(entry.period||'per period')}</small></div>`,'Metric'); }
  return shell(entry,`<div class="metric-value">${value}</div><div class="metric-meta"><span class="state good">▲ ${num(entry.delta,'6.4')}%</span><span>vs target</span></div><div class="meter"><span style="width:84%"></span></div>`,'Metric');
}
...
function comparison(entry){
  ...
  if(n.includes('time compression')) return shell(entry,`<div class="time-compression-live"><div><i></i><i></i><i></i><i></i>${labeled(entry.before,'188','Before')}</div><span>→</span><div class="short"><i></i>${labeled(entry.after,'14','After')}</div></div>`,'Comparison');
  if(n.includes('reduction')) return shell(entry,`<div class="reduction-visual"><div>${labeled(entry.before,'188','Before')}</div><i>→</i><div>${labeled(entry.after,'14','After')}</div></div>${unit?`<small class="comparison-unit-label">Unit · ${unit}</small>`:''}`,'Comparison');
  ...
  if(n.includes('quality')) return shell(entry,`<div class="quality-shift"><span style="--v:${Math.max(0,Math.min(100,Number(entry.before??62)||0))}%">${num(entry.before,'62%')}</span><span style="--v:${Math.max(0,Math.min(100,Number(entry.after??91)||0))}%">${num(entry.after,'91%')}</span></div><div class="metric-meta"><span>Before</span><span>After</span></div>`,'Comparison');
  if(n.includes('as-is')) return shell(entry,`<div class="asis-tobe"><div><b>As-is</b><span>${num(entry.before,'Current')}${unit?` <small class="comparison-unit">${unit}</small>`:''}</span></div><i>→</i><div><b>To-be</b><span>${num(entry.after,'Future')}${unit?` <small class="comparison-unit">${unit}</small>`:''}</span></div></div>`,'Comparison');
  if(n.includes('before/after')) return shell(entry,`<div class="before-after-kpi"><div><small>Before</small><b>${num(entry.before,'62')}${unit?` <small class="comparison-unit">${unit}</small>`:''}</b></div><div><small>After</small><b>${num(entry.after,'91')}${unit?` <small class="comparison-unit">${unit}</small>`:''}</b><em>▲</em></div></div>`,'Comparison');
...
function table(entry){
  const n=entry.element.toLowerCase();
  const bound=entry.customTable;
  if(bound?.headers?.length){
    const meaningful=(bound.rows||[]).some(row=>Array.isArray(row)&&row.some(value=>value!==null&&value!==undefined&&String(value).trim()!==''));
    if(!meaningful)return shell(entry,`<div class="table-frame table-scroll empty-data"><table><thead><tr>${bound.headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead></table><div class="data-empty-state"><b>No rows yet</b><span>Paste data or open Data in the inspector.</span></div></div>`,'Table');
    return shell(entry,`<div class="table-frame table-scroll"><table><thead><tr>${bound.headers.map((h,i)=>`<th data-direct="${entry.dataset_id?'dataset-header':'table-header'}:${i}" title="Double-click to rename">${esc(h)}</th>`).join('')}</tr></thead><tbody>${(bound.rows||[]).map((row,rowIndex)=>`<tr>${bound.headers.map((_,i)=>`<td data-direct="${entry.dataset_id?'dataset-cell':'table-cell'}:${rowIndex}:${i}" title="Double-click to edit">${num(row[i],'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,'Table');
  }
  if(entry.dataset_id&&entry.customTable?.headers?.length) return shell(entry,`<div class="table-frame table-scroll"><table><thead><tr>${entry.customTable.headers.map((h,i)=>`<th data-direct="dataset-header:${i}" title="Double-click to rename">${esc(h)}</th>`).join('')}</tr></thead><tbody>${(entry.customTable.rows||[]).map((row,r)=>`<tr>${entry.customTable.headers.map((_,i)=>`<td data-direct="dataset-cell:${r}:${i}" title="Double-click to edit">${num(row[i],'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,'Table');
  ...
  if(c?.headers?.length){
    ...
    if(!meaningful) return shell(entry,`<div class="table-frame table-scroll empty-data"><table><thead><tr>${c.headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead></table><div class="data-empty-state"><b>No rows yet</b><span>Paste data or open Data in the inspector.</span></div></div>`,'Table');
    return shell(entry,`<div class="table-frame table-scroll"><table><thead><tr>${c.headers.map((h,i)=>`<th data-direct="table-header:${i}" title="Double-click to rename">${esc(h)}</th>`).join('')}</tr></thead><tbody>${(c.rows||[]).map((r,rowIndex)=>`<tr>${c.headers.map((_,i)=>`<td data-direct="table-cell:${rowIndex}:${i}" title="Double-click to edit">${num(r[i],'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,'Table');
  }
  ...
  return shell(entry,`<div class="table-frame"><table><tbody>${rows.map(row=>`<tr>${(Array.isArray(row)?row:Object.values(row||{})).slice(0,4).map(v=>`<td>${num(v,'')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,'Table');
}
...
function image(entry){
  const n=entry.element.toLowerCase(),src=entry.src?`background-image:url('${String(entry.src).replace(/'/g,'%27')}');background-size:${entry.fit==='fit'?'contain':'cover'};background-position:${esc(entry.focal||'50% 50%')};background-repeat:no-repeat;`:'',alt=esc(String(entry.alt??'').trim()||(entry.src?'Image':'Add image')),imageId=`${slug(entry.id||entry.element)}-image`,captionId=`${imageId}-caption`;
  ...
  if(n==='screenshot frame') return shell(entry,`<div class="screenshot-frame-live"><header><i></i><i></i><i></i><span>Application</span></header><div style="${src}" role="img" aria-label="${alt}"><b>${entry.src?'':'Add screenshot or mockup'}</b></div></div>`,'Media');
  if(n.includes('image + caption')) return shell(entry,`<figure class="captioned-image-live"><div id="${imageId}" style="${src}" role="img" aria-label="${alt}" aria-describedby="${captionId}">${entry.src?'':'Add image'}</div><figcaption id="${captionId}" data-direct="caption" title="Double-click to edit caption">${esc(entry.caption||'Caption: describe the image')}</figcaption></figure>`,'Media');
  ...
  return shell(entry,`<div class="image-stage ${n.includes('hero')?'hero-media':''}" style="${src}" role="img" aria-label="${alt}">${entry.src?'':`<span>Add image</span>`}</div>${entry.caption?`<p class="image-caption">${esc(entry.caption)}</p>`:''}`,'Media');
}
...
function composite(entry){
  ...
  if(kind==='EvidenceCompositeEngine'){
    ...
    if(n==='evidence card') return shell(entry,`<div class="evidence-card-live"><header><i class="good"></i><b>${statement}</b></header><p>${detail}</p><footer><span>Evidence</span><em>${esc(entry.status||'Observed')}</em></footer></div>`,'Evidence');
  ...
  if(kind==='DecisionCompositeEngine'){
    ...
    if(n.includes('risk callout')) return shell(entry,`<div class="risk-callout-live"><i>!</i><div><b>${statement}</b><p>${detail}</p></div><span>${esc(entry.status||'Open')}</span></div>`,'Decision');
  ...
  if(kind==='ProjectCompositeEngine'){
    if(n==='project card') return shell(entry,`<div class="project-card-live"><header><b>${statement}</b><span>${esc(entry.status||'Unspecified')}</span></header><p>${detail}</p></div>`,'Project');
...
function wafer(entry){
  ...
  if(Array.isArray(entry.observations)&&entry.observations.length) { const observed=new Map(entry.observations.map(row=>[`${row.x}:${row.y}`,row.value])); const values=entry.observations.map(row=>row.value).filter(value=>typeof value==='number'&&Number.isFinite(value));if(!values.length)return shell(entry,'<div class="data-empty-state full"><b>No numeric die values</b><span>Map a numeric measurement or yield field to the wafer value role.</span></div>','Fab');const min=Math.min(...values),max=Math.max(...values),range=max-min,level=value=>typeof value==='number'&&Number.isFinite(value)?Math.max(0,Math.min(4,range===0?2:Math.round((value-min)/range*4))):0;const cells=[];for(let y=-4;y<=4;y+=1)for(let x=-5;x<=5;x+=1)if(x*x+y*y<28){const value=observed.get(`${x}:${y}`);cells.push(`<span class="die heat-${level(value)}" style="--x:${x+5};--y:${y+4}" title="${esc(value??'Missing')}"></span>`);}const routeField=fieldName('route'),routeValue=routeField&&fabRows.length?fabRows.find(row=>row[routeField]!==null&&row[routeField]!==undefined&&String(row[routeField]).trim()!=='')?.[routeField]:null;const identity=[['Wafer',entry.wafer_id],['Lot',entry.lot],['Tool',entry.tool],['Chamber',entry.chamber],['Route',routeValue],['Recipe',entry.recipe],['Step',entry.process],['Bin',entry.bin]].filter(([,value])=>value!==null&&value!==undefined&&String(value).trim()!=='');return shell(entry,`<div class="wafer-demo"><div class="wafer-grid">${cells.join('')}</div><span class="wafer-notch"></span></div><div class="metric-meta"><span>${num(min)} → ${num(max)}${range===0?' · constant population':''}</span>${identity.map(([label,value])=>`<span>${esc(label)} · ${esc(value)}</span>`).join('')}</div>`,'Fab'); }
...
function renderLegacyNonCoreChart(entry){
  ...
  const labelIndexes=valid.length<=6?valid.map((_,index)=>index):[0,Math.round((valid.length-1)/2),valid.length-1].filter((index,pos,all)=>all.indexOf(index)===pos);
  const categoryLabel=value=>{const text=String(value??'');return text.length>12?`${text.slice(0,11)}…`:text;};
  ...
  if(n.includes('bar')||n.includes('histogram')||n.includes('pareto')||n.includes('waterfall')){
    ...
    const staticContext=n==='vertical bar'?`<g class="chart-static-context">${labelIndexes.map(index=>{const row=valid[index],x=left+(index+.5)*plotW/Math.max(1,valid.length),h=row.v===null?0:Math.abs(row.v)/max*plotH;return `<text class="chart-value-label" x="${x.toFixed(1)}" y="${Math.max(10,top+plotH-h-4).toFixed(1)}" text-anchor="middle">${num(row.v,'')}</text><text class="chart-axis-label" x="${x.toFixed(1)}" y="${H-5}" text-anchor="middle">${esc(categoryLabel(row.label))}</text>`;}).join('')}</g>`:'';
    return shell(entry,`<svg class="viz-svg" viewBox="0 0 ${W} ${H}"><path class="gridline" d="M${left} ${top+plotH}H${W-right} M${left} ${top+plotH*.5}H${W-right}"/>${bars}${pareto}${staticContext}</svg>`,'Chart');
  }
  if(n.includes('scatter')||n.includes('bubble')||n.includes('strip')||n.includes('dot')){
    ...
    const dots=valid.map((r,i)=>{const x=left+(i+.5)*plotW/Math.max(1,valid.length),y=r.v===null?top+plotH:yFor(r.v);return `<circle data-behavior-point="${i}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${n.includes('bubble')?4+(i%4)*1.5:4}" class="accent-fill"/>`;}).join('');
  ...
  const staticContext=(n==='line chart'||n==='area chart')?`<g class="chart-static-context"><text class="chart-axis-label" x="${left-4}" y="${top+5}" text-anchor="end">${num(Math.max(...nums))}</text><text class="chart-axis-label" x="${left-4}" y="${top+plotH}" text-anchor="end">${num(Math.min(...nums))}</text>${labelIndexes.map(index=>{const point=points[index],row=valid[index];return `<text class="chart-axis-label" x="${point.x.toFixed(1)}" y="${H-5}" text-anchor="middle">${esc(categoryLabel(row.label))}</text>${point.y===null?'':`<text class="chart-value-label" x="${point.x.toFixed(1)}" y="${Math.max(10,point.y-5).toFixed(1)}" text-anchor="middle">${num(point.value)}</text>`}`;}).join('')}</g>`:'';
  return shell(entry,`<svg class="viz-svg" viewBox="0 0 ${W} ${H}"><path class="gridline" d="M${left} ${top+plotH}H${W-right} M${left} ${top+plotH*.5}H${W-right}"/>${lines}${points.filter(p=>p.y!==null).map((p,index)=>`<circle data-behavior-point="${p.sourceIndex}" aria-label="${esc(p.label)}: ${num(p.value)}" cx="${p.x}" cy="${p.y}" r="3" class="accent-fill"><title>${esc(p.label)}: ${num(p.value)}</title></circle>`).join('')}${staticContext}</svg>`,'Chart');
}

function engineeringEmpty(entry,n,W,H){
  const required=n.includes('main effects')||n.includes('interaction')?'categorical factors and a numeric response':n.includes('surface')||n.includes('contour')?'two numeric factors and a numeric response':n.includes('confidence')?'a category and repeated numeric values':n.includes('residual')||n.includes('predicted')?'numeric features and a numeric response':'at least two numeric observations';
  return shell(entry,`<div class="data-empty-state full"><b>Insufficient data for ${esc(entry.element)}</b><span>Map ${esc(required)}. Specification limits are optional and remain separate from observations.</span></div>`,'Engineering');
...
function engineering(entry){
  const n=entry.element.toLowerCase(),obs=Array.isArray(entry.observations)?entry.observations:[],values=obs.map(x=>typeof x?.value==='number'?x.value:null),nums=values.filter(x=>x!==null),W=270,H=122,left=20,right=10,top=10,bottom=20,pw=W-left-right,ph=H-top-bottom;
  ...
  if(type&&nums.length>=2){try{
    const options={title:entry.title||entry.element};
    if(type==='spc')Object.assign(options,{lsl:finite(entry.specification_low)?entry.specification_low:null,usl:finite(entry.specification_high)?entry.specification_high:null});
    const decisionH=finite(entry.decision_h)?entry.decision_h:(!finite(entry.w)&&finite(entry.h)?entry.h:5);
    if(type==='cusum')Object.assign(options,{target:finite(entry.target)?entry.target:null,sigma:finite(entry.sigma)?entry.sigma:null,k:finite(entry.k)?entry.k:0.5,h:decisionH});
    if(type==='ewma')Object.assign(options,{target:finite(entry.target)?entry.target:null,sigma:finite(entry.sigma)?entry.sigma:null,lambda:finite(entry.lambda)?entry.lambda:0.2,L:finite(entry.L)?entry.L:3});
    const plan=prepareEngineeringChart(type,type==='xbarr'?{subgroups:entry.subgroups}:{values:nums},options);
    return shell(entry,renderEngineeringChartSvg(plan,{width:300,height:150}),'Engineering');
  }catch{/* invalid analytical input falls through to a visible non-analytical state */}}
  if(nums.length<2 && !rows.length) return engineeringEmpty(entry,n,W,H);
  ...
  const lcl=entry.lcl??entry.lower_limit,ucl=entry.ucl??entry.upper_limit,lsl=entry.specification_low,usl=entry.specification_high;
  const scaleValues=nums.concat([lcl,ucl,lsl,usl].filter(value=>typeof value==='number'&&Number.isFinite(value))),min=Math.min(...scaleValues),range=Math.max(...scaleValues)-min,yFor=value=>range===0?top+ph/2:top+ph-(value-min)/range*ph;
  const pts=values.map((v,i)=>({x:left+i*pw/Math.max(1,values.length-1),y:v===null?null:yFor(v)}));
  const path=pts.filter(p=>p.y!==null).map((p,i)=>`${i?'L':'M'}${p.x.toFixed(1)} ${p.y.toFixed(1)}`).join(' ');
  const limit=v=>typeof v==='number'?yFor(v):null;
  return shell(entry,`<svg class="viz-svg engineering-live" viewBox="0 0 ${W} ${H}"><path class="gridline" d="M${left} ${top+ph}H${W-right} M${left} ${top+ph*.5}H${W-right}"/>${limit(ucl)!==null?`<path d="M${left} ${limit(ucl)}H${W-right}" class="limit-line"/>`:''}${limit(lcl)!==null?`<path d="M${left} ${limit(lcl)}H${W-right}" class="limit-line"/>`:''}${limit(usl)!==null?`<path d="M${left} ${limit(usl)}H${W-right}" class="spec-line"/>`:''}${limit(lsl)!==null?`<path d="M${left} ${limit(lsl)}H${W-right}" class="spec-line"/>`:''}<path d="${path}" class="accent-line"/>${pts.filter(p=>p.y!==null).map(p=>`<circle cx="${p.x}" cy="${p.y}" r="3.3" class="accent-fill"/>`).join('')}</svg><div class="metric-meta"><span>${obs.length} observations</span><span>${esc(entry.role||'measurement')}</span><span>Control ${num(lcl,'—')}–${num(ucl,'—')} · Spec ${num(lsl,'—')}–${num(usl,'—')}</span></div>`,'Engineering');
}
...
function interaction(entry){
  ...
  if(n.includes('tabs')) { const tabs=['Overview','Evidence','Actions'],active=tabs.includes(entry.tab)?entry.tab:tabs[0],copy={Overview:'Current investigation state and decision context.',Evidence:'Supporting and contradictory evidence remain visible.',Actions:'Next owner action and due date.'}; return shell(entry,`<div class="tabs-live" role="tablist">${tabs.map(tab=>`<button type="button" data-tab="${tab}" role="tab" aria-selected="${tab===active}">${esc(tab)}</button>`).join('')}</div><div class="tab-surface-live" role="tabpanel">${esc(copy[active])}</div>`,'Interaction'); }
  if(n.includes('expand')) return shell(entry,`<button type="button" class="expand-live" data-action="expand" aria-expanded="${entry.expanded===true}"><b>Root cause evidence</b><span>${entry.expanded===true?'⌃':'⌄'}</span></button>${entry.expanded===true?'<div class="expand-detail-live">Pressure excursion aligns with defect onset.</div>':''}`,'Interaction');
  ...
  if(n.includes('timeline')) { const points=['Collect','Analyze','Decide'],active=Number.isInteger(entry.tm)&&entry.tm>=0&&entry.tm<points.length?entry.tm:0; return shell(entry,`<div class="interactive-timeline-live">${points.map((label,index)=>`<button type="button" data-tm="${index}" aria-pressed="${index===active}" class="${index===active?'active':''}"><i></i><span>${label}</span></button>`).join('')}</div>`,'Interaction'); }
