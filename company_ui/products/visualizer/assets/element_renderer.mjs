...
function table(entry){
  ...
  if(c?.headers?.length){
    ...
    if(!meaningful) return shell(entry,tableRegion(entry,`<table><thead><tr>${c.headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead></table><div class="data-empty-state"><b>No rows yet</b><span>Paste data or open Data in the inspector.</span></div>`),'Table');
    return shell(entry,tableRegion(entry,`<table><thead><tr>${c.headers.map((h,i)=>`<th data-direct="table-header:${i}" title="Double-click to rename">${esc(h)}</th>`).join('')}</tr></thead><tbody>${(c.rows||[]).map((r,rowIndex)=>`<tr>${c.headers.map((_,i)=>`<td data-direct="table-cell:${rowIndex}:${i}" title="Double-click to edit">${num(r[i],'')}</td>`).join('')}</tr>`).join('')}</tbody></table>`),'Table');
  }
