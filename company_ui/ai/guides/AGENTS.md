...
## V3.0 APPLICATION-PLATFORM ALPHA LAW
- `3.0.0a1` inherits all established v2/v1.7 visual and runtime contracts as the compatibility floor; v3 application-platform capability does not authorize arbitrary redesign.
- `company_ui/release_authority.json` is the release identity authority. Governance must pass before packaging.
- V3 runtime/data/workspace/visualization/extension services are additive and opt-in; existing v2 routes must not be forced onto new layout or renderer anatomy.
- Root public exports recorded in `PUBLIC_API_CONTRACT.json` are governed; this alpha is additive to the inherited surface and must not silently delete or redefine established APIs.
- Main geometry remains 10px control / 14px surface / 18px overlay; other legitimate geometry must use named tokens.
- Application-level layering must use canonical layer tokens; do not solve conflicts with large literal z-index values.
- Shared analytical state must use governed runtime/data-session ownership rather than diverging component-local copies.
- Semantic visualization must reuse the certified visual renderer stack unless a separately governed replacement is explicitly approved.
- Browser certification uses canonical responsive widths 390/430/768/1024/1280/1440.
- Stable 3.0.0 promotion is forbidden until installed-runtime contract, real 22-route smoke, browser matrix, human visual baseline approval and required target-platform evidence pass in the company environment.
