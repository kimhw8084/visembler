...
def test_refresh_connector_unchanged_and_editor_has_shared_replace_guard():
 js=(ROOT/'company_ui/products/visualizer/assets/integrated_editor.mjs').read_text();assert 'function commitDatasetRefresh' in js and 'datasetConsumers(existing.id).length>1' in js and 'source_dataset_id:detached.id' in js and 'Schema changed · ${detail}' in js and "linked===1?'Refresh data'" in js and "$('#refreshOnly')?.addEventListener" in js
